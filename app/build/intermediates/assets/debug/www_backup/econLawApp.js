...
Object.assign(app, {
    ...
    renderEconLawGrid(module = 'all', search = '') {
        ...
        container.innerHTML = knowledge.map(item => {
            ...
            if (lastRecord) {
                ...
                statusBadge = `<span class="knowledge-status ${status.class}">${status.emoji} ${status.text}</span>`;
            } else {
                statusBadge = `<span class="knowledge-status status-none">未学习</span>`;
            }
            ...
            return `
                <div class="knowledge-card" onclick="app.showEconLawDetail('${item.id}')">
                    ...
                    <p class="knowledge-preview">${contentPreview ? contentPreview.substring(0, 60) : ''}...</p>
                    <div class="knowledge-footer">
                        <div class="memory-strength">
                            ${[1,2,3,4,5].map(i => `<span class="strength-dot ${i <= strength/20 ? 'active' : ''}"></span>`).join('')}
                        </div>
                        ${item.examYears ? `<span class="exam-years">真题：${item.examYears.join('、')}</span>` : ''}
                    </div>
                </div>
            `;
        }).join('');
        ...
        document.getElementById('modal-body').innerHTML = `
            ...
            <div class="modal-section">
                ...
                <div class="feedback-buttons" style="justify-content: flex-start;">
                    <button class="feedback-btn fuzzy" onclick="(function(){ app.studyKnowledge('${id}', 'fuzzy'); app.closeModal(); app.renderEconLawGrid && app.renderEconLawGrid(app.currentEconLawModule || 'all'); })();">
                        <span>😵</span> 没记住
                    </button>
                    <button class="feedback-btn normal" onclick="(function(){ app.studyKnowledge('${id}', 'normal'); app.closeModal(); app.renderEconLawGrid && app.renderEconLawGrid(app.currentEconLawModule || 'all'); })();">
                        <span>😐</span> 一般
                    </button>
                    <button class="feedback-btn mastered" onclick="(function(){ app.studyKnowledge('${id}', 'mastered'); app.closeModal(); app.renderEconLawGrid && app.renderEconLawGrid(app.currentEconLawModule || 'all'); })();">
                        <span>😎</span> 已掌握
                    </button>
                </div>
            </div>
            <div class="modal-section" style="border-top: 1px solid var(--border); padding-top: 16px; margin-top: 16px;">
                <h4>⚙️ 管理</h4>
                <div style="display: flex; gap: 10px;">
                    <button class="vocab-btn" onclick="app.showEditKnowledgeModal('${id}')" style="flex: 1;">
                        ✏️ 编辑
                    </button>
                    ${isCustom ? `
                        <button class="vocab-btn" onclick="app.deleteKnowledge('${id}')" style="flex: 1; background: var(--danger);">
                            🗑️ 删除
                        </button>
                    ` : ''}
                </div>
    ...
    studyEconLaw(id, status) {
        console.log('[studyEconLaw] id:', id, 'status:', status);
        // 调用 app.js 中的 studyKnowledge
        app.studyKnowledge(id, status);
        // 关闭弹窗
        app.closeModal();
        // 刷新网格以更新状态显示
        this.renderEconLawGrid(this.currentEconLawModule);
    },
    
    // 显示独立学习详情页面
    showStudyDetail(id) {
        console.log('[showStudyDetail] Showing detail for:', id);
        const item = econLawData.knowledge.find(k => k.id === id);
        if (!item) {
            console.error('[showStudyDetail] Item not found:', id);
            return;
        }
        ...
        // 更新标题
        document.getElementById('study-detail-module').textContent = moduleInfo.name;
        
        // 生成内容
        document.getElementById('study-detail-content').innerHTML = `
            <div class="study-detail-type" style="background: ${moduleInfo.color}">${this.getTypeName(item.type)}</div>
            <h3 class="study-detail-question">${item.title}</h3>
            ${item.examYears ? `<p class="study-detail-meta">真题：${item.examYears.join('、')}年</p>` : ''}
            
            <div class="study-detail-section">
                <h4>📝 标准答案</h4>
                <div class="study-detail-answer">${content}</div>
            </div>
            
            ${item.tip ? `
                <div class="study-detail-section tip">
                    <h4>💡 记忆技巧</h4>
                    <p>${item.tip}</p>
                </div>
            ` : ''}
            ...
            <div class="study-detail-section">
                <h4>🎯 掌握程度</h4>
                <div class="mastery-level-buttons">
                    <button class="mastery-btn fuzzy" onclick="app.setMasteryLevel('${id}', 'fuzzy')">
                        <span class="mastery-emoji">😵</span>
                        <span class="mastery-label">陌生</span>
                    </button>
                    <button class="mastery-btn normal" onclick="app.setMasteryLevel('${id}', 'normal')">
                        <span class="mastery-emoji">😐</span>
                        <span class="mastery-label">一般</span>
                    </button>
                    <button class="mastery-btn mastered" onclick="app.setMasteryLevel('${id}', 'mastered')">
                        <span class="mastery-emoji">😎</span>
        ...
        // 保存当前学习项ID
        this.currentStudyItem = id;
        
        // 切换到学习详情页面
        this.switchPage('study-detail');
    },
    ...
    // 设置掌握程度
    setMasteryLevel(id, status) {
        console.log('[setMasteryLevel] id:', id, 'status:', status);
        
        // 调用学习记录功能（使用 app. 确保正确引用）
        app.studyKnowledge(id, status);
        ...
        const selectedBtn = document.querySelector(`.mastery-btn.${status}`);
        if (selectedBtn) {
        ...
        const messages = {
            fuzzy: '已标记为陌生，需要加强复习 💪',
            normal: '已标记为一般，继续巩固 📚',
            mastered: '已标记为已掌握，继续保持 🎉'
        };
        app.showToast(messages[status]);
    },
